...
export async function processTaskDueReminders(): Promise<ReminderRunStats> {
  ...
  for (const t of tasks as any[]) {
    ...
    if (taskYmd && taskAssignee) {
      let label: 'overdue' | 'due_today' | 'due_tomorrow' | null = null;
      if (taskYmd < today) label = 'overdue';
      else if (taskYmd === today) label = 'due_today';
    ...
    for (const row of normalizeChecklistRows(t.journal_logs)) {
      ...
      let label: 'overdue' | 'due_today' | 'due_tomorrow' | null = null;
      if (ymd < today) label = 'overdue';
      else if (ymd === today) label = 'due_today';
      ...
      await notificationDb.create({
        ...
        title:
          label === 'overdue'
            ...
            : label === 'due_today'
              ? 'Checklist item due today'
              : 'Checklist item due tomorrow',
        message: `"${title}" — ${text}`,
        link: openLink,
      });
      await sendPushToUser(assignee, {
        title: label === 'overdue' ? 'Checklist overdue' : label === 'due_today' ? 'Checklist due today' : 'Checklist due tomorrow',
