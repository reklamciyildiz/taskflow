...
export async function GET(
  ...
  try {
    ...
    const membership = await teamMemberDb.getMembership(
      (task as any).team_id,
      user.id,
    );
...
export async function PATCH(
  ...
  try {
    ...
    // We enforce this for both task-level reminders and checklist-row reminders inside journal_logs.
    {
      ...
      if (
        ...
        const nextReminders = Array.isArray(body.reminders)
          ? body.reminders.filter(
              (x: any) => typeof x === "string" && x.length > 0,
            )
          : [];
        if (nextReminders.length > 0) {
          const dueSource = Object.prototype.hasOwnProperty.call(
            body ?? {},
            "dueDate",
          )
            ? body.dueDate
    ...
    if (
      ...
      try {
        await notificationDb.deleteChecklistNotifications(
          ...
          undefined,
          ["task_due_reminder", "task_reminder"],
        );
      } catch (err) {
    ...
    if (Array.isArray(body.journalLogs) && actor?.id) {
      ...
      const projectId = (updatedTask as any).project_id
        ? String((updatedTask as any).project_id)
        : null;
      ...
      for (const oldId of oldMap.keys()) {
        ...
        if (!newMap.has(oldId)) {
          ...
          try {
            ...
            console.error(
              "Failed to cleanup deleted checklist notifications",
              e,
            );
          }
        }
          ...
          if (prevReminders !== nextReminders || prevDue !== nextDue) {
            try {
              await notificationDb.deleteChecklistNotifications(
                String(updatedTask.id),
                id,
                ["checklist_due_reminder", "checklist_reminder"],
              );
            } catch (err) {
              console.error(
                "Failed to cleanup revised checklist reminders",
                err,
              );
            }
          }
    ...
    try {
      ...
      if (
        ...
        body.description !== originalTask.description
      ) {
        changes.push({
          field: "description",
      ...
      if (
        ...
        body.priority !== originalTask.priority
      ) {
        changes.push({
          field: "priority",
      ...
      if (body.status === "done" && originalTask.status !== "done") {
        const completedByUserId = session?.user
        ...
        if (teamId) {
          try {
            ...
            console.error(
              "Failed to create team activity notifications:",
              teamActivityError,
            );
          }
        }
    ...
    try {
      ...
      if (
        ...
        originalTask.assignee_id !== nextAssigneeId
      ) {
        void removeGoogleCalendarForUserTask({
          userId: originalTask.assignee_id,
